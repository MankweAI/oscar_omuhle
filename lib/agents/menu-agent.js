...
class MenuAgent {
  ...
  async processMessage(userMessage, session) {
    ...
    if (agentState.stage === STAGES.HOME) {
      ...
      if (
        ...
        // Poster Text Placeholder + Call to Action
        response =
        ...
        response =
          ...
          `_Tickets available exclusively on WhatsApp._\n` + // <--- FIXED
          `Reply *MENU* to go back.`;
  ...
  getMainMenu() {
    return (
      ...
      `3️⃣ 🚐 *Upcoming Gigs* (Ermelo / Menlyn)\n` +
